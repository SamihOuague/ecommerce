...
const CheckoutResources = ({setInfos}) => {
    return (
        <Resources path={`${process.env.REACT_APP_API_URL}${process.env.REACT_APP_AUTH}/get-user`} render={(data) => {
...
const PaymentResources = ({ infos }) => {
    return <Resources path={`${process.env.REACT_APP_API_URL}${process.env.REACT_APP_ORDER}/config`} render={(data) => {
        if (data.loading) return <Spinner />;
...
const ConfirmationResources = () => {
    ...
    return <Resources path={`${process.env.REACT_APP_API_URL}${process.env.REACT_APP_ORDER}/confirm-order/${id}`} render={(data) => {
        if (data.loading) return <Spinner />;
